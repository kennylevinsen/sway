...
static void handle_frame(struct wl_listener *listener, void *user_data) {
	...
	if (output->max_render_time_ns != 0) {
		...
		int64_t now_ns = now.tv_nsec + now.tv_sec * NSEC_IN_SECONDS;
		int64_t last_ns = output->last_presentation.tv_nsec
			+ output->last_presentation.tv_sec * NSEC_IN_SECONDS;
		nsec_until_refresh = last_ns + output->refresh_nsec - now_ns;
	}
